...
def run_episode(brain, env, ep_idx, task, place=None, biletaxis=None, olf=False, seq=None):
    ...
    while not done:
        ...
        if seq is not None and getattr(seq, "goal_beacon", False):
            import numpy as _np
            tx, ty = seq.target()
            ax_, ay_ = env.agent_x / env.config.width, env.agent_y / env.config.height
            gang = _np.arctan2(ty - ay_, tx - ax_)
            rel = (gang - env.agent_angle + _np.pi) % (2 * _np.pi) - _np.pi  # [-π,π], >0=CCW(왼쪽)
            nh = env.config.n_rays // 2
            lval = 0.9 if rel > 0 else 0.2
            rval = 0.9 if rel <= 0 else 0.2
            obs["good_food_rays_left"] = _np.ones(nh) * lval
            obs["good_food_rays_right"] = _np.ones(nh) * rval
            obs["food_rays_left"] = _np.ones(nh) * lval
            obs["food_rays_right"] = _np.ones(nh) * rval
