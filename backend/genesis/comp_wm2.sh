...
SEED=${1:-0}
echo "=== 조합+창발WM(seq-nav 없이) seed$SEED ==="
python -u $S --task integrated --context-select --seq-task --seq-wm --context-compositional --zone-cx 0.3 --zone-cy 0.3 --episodes 20 --seed $SEED 2>&1 | grep -iE "compositional\]|comp ctx|WM latch|Traceback" | tail -4
echo "=== END ==="
