...
## 강화 시도 = 천장 확인 (danger_food_ratio 0.6, 60ep, 2026-08-01)
가설: 위험-음식 결합 2배(0.3→0.6)로 훈련하면 합성 강해질까. brain_compositional_60ep.npz, graded 재프로브.
결과 (0.10 위험, 2회): **+21 / +27%pt** — 250ep(ratio 0.3)의 +23~25%pt와 **동일 천장.**
- 위험노출 2배로도 합성 안 커짐. 60ep인데 250ep와 같은 수준 = **같은 천장에 더 빨리 닿을 뿐.**
- brave-for-good 여전 소수(~25%). **값의 위험감수도 조절이 ~+20~27%pt서 포화.**
- 해석: 하드와이어 위험회피(Pain→Motor push=60/pull=-40)가 학습된 값 영향을 캡. 노출 증가로 못 넘음.
- 더 강화하려면 반사/값 균형 조정(아키텍처) 필요 = 하드튜닝 경계 → 안 함. **천장 = 정직한 한계.**

## 결론: 4번째 개념층위 = 약한 graded 합성 (real, qualified, 천장 확인)
