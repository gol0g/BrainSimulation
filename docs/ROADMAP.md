...
## 현재 상태: KC 3000 다중감각 통합 (27,910 뉴런, 800×800 맵)

```
╔═══════════════════════════════════════════════════════════════╗
║  KC 3000 + 9개 다중감각 입력 (2026-03-28)                       ║
║  KC 1500→3000 확장, 9개 입력:                                    ║
║    시각(food/good/bad) + IT + Assoc + PPC + Social + Wernicke   ║
║  800×800맵, 장애물 1개, Rich Zones 2개, Temporal shift            ║
║  Phase 12-20 전체 활성 (V1→IT→STS→PPC→Assoc→WM→Meta→Self)      ║
║  100ep: 53% 생존, Selectivity 0.68, Reward 2.53%               ║
╚═══════════════════════════════════════════════════════════════╝
```
...
| **E3** | **Temporal shift (rich zone 이동)** | **0** | **✓ 완료** | 70% |
| **L17** | **Social→KC + 청각 활성화** | **0** | **✓ 완료** | 45-70% |
| **L18** | **고차 피질→KC (Assoc+PPC→BG)** | **0** | **✓ 완료** | **64%** |
| **KC2** | **KC 1500→3000 + 다중감각 9입력** | **+3,400** | **✓ 완료** | **53%** |
| **Aud** | **청각 입력 활성화 (sensitivity 20)** | **0** | **✓ 완료** | |
| **Lang** | **Wernicke_Food→KC (언어→BG)** | **0** | **✓ 완료** | |
...
| **시간 변화** | **rich zone 1500스텝마다 이동** | ⭐⭐⭐⭐ | **E3** | **✓** |
| 장기 | 다중 에이전트 | ⭐⭐⭐⭐ | 사이드 | 계획 |
| 최종 | 언어 환경 | ⭐⭐⭐⭐⭐ | C3 | 계획 |
...
> 생존 강화는 완료. 이제 "경험에서 개념을 형성하는 뇌"를 향한 경로.

### C0: 개념 검증 계측 (NEXT)

"개념이 생겼다"를 판정하는 지표 프레임워크. Phase 검증과 동일한 엄격함 적용.

| 지표 | 측정 방법 | 기준 |
|------|----------|------|
| **다중감각 범주화** | 같은 시각이지만 다른 소리/맥락 → 행동이 달라지는가 | 정확도 >70% |
| **범주 일반화** | 처음 보는 변형 샘플에도 범주 적용되는가 | 신규 자극 정확도 >50% |
...
IT/Assoc를 scaffold로 유지하면서 KC/Assoc_Binding에서 새 범주 창발.

1. 기존 IT/Assoc는 scaffold로 유지 (제거하지 않음)
2. KC/Assoc_Binding이 다중감각+문맥 묶음을 학습
3. 행동 제어가 새 범주에 실리면
