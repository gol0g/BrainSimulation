...
# 6. 식별가능성 (2026-09-18 신설). E082는 불변식을 다 지키고도 질문에 답할 수 없는 대조였다.
#    내부적으로 일관된 선언을 달고 게이트를 통과한 뒤 20런을 태웠다.
#    완결성·불변식 검사만으로는 **대조 설계의 결함**을 못 잡는다.
grep -q '^## 6\. 이 대조가' "$F" || fail "6. 식별가능성 절이 없다 (E082: 대조가 관심 요인을 상쇄해 20런 폐기)"
grep -q '<실험군에만 있는 것을' "$F" && fail "6. 두 조건의 차이 목록이 비어 있다"
grep -q '<나머지는 교란이다' "$F" && fail "6. 의도한 차이가 명시되지 않았다"
