...
echo "=== D20 긴 eligibility trace(0.99) seq 25ep ==="
python -u "$RUN" --task integrated --seq-task --seq-nav --seq-wm --seq-pattern-latch \
  --inhib-wm -200 --trace-decay 0.99 --zone-cx 0.3 --zone-cy 0.3 --sparse-reward --n-food 4 \
  --biletaxis --biletaxis-gain 0.5 --biletaxis-brake --episodes 25 --seed 0 \
  --output "$D/d20_trace99.json" 2>&1 | tee "$LOG" \
