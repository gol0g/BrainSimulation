#!/bin/bash
# E074: P12 설계(환경 고정 + 뇌 반복)로 H004 재판정.
set -u
...
for E in 0 1 2; do
  ...
  for B in 0 1 2 3 4; do
    for CFG in "기준1.0x 0.08 0.15" "11.25x 0.30 0.05"; do
      ...
      timeout 3600 python reflex_override_task.py $COMMON --brain-seed "$B" --env-seed "$E" \
        --learn-sparsity "$2" --reflex-sparsity "$3" 2>&1 | grep -E "^=>"
      printf "  env%s b%s %-9s 정적: " "$E" "$B" "$1"
      timeout 3600 python reflex_override_task.py $STATIC --brain-seed "$B" --env-seed "$E" \
        --learn-sparsity "$2" --reflex-sparsity "$3" 2>&1 | grep -E "^=>"
    done
  done
done
