...
echo "=== D24floor 무작위목표+easy-survival 25ep ==="
python -u "$RUN" --task integrated --seq-task --seq-nav --seq-wm --seq-pattern-latch --seq-random-target --easy-survival \
  --inhib-wm -200 --zone-cx 0.3 --zone-cy 0.3 --sparse-reward --n-food 4 --episodes 25 --seed 0 \
  --output "$D/d24floor_random_easy.json" 2>&1 | tee /tmp/d24floor_seq.log \
