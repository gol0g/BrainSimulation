...
echo "=== D22 goal-beacon 뇌구동 항법 25ep ==="
python -u "$RUN" --task integrated --seq-task --seq-nav --seq-wm --seq-pattern-latch --seq-goal-beacon \
  --inhib-wm -200 --zone-cx 0.3 --zone-cy 0.3 --sparse-reward --n-food 4 \
  --episodes 25 --seed 0 \
  --output "$D/d22_beacon.json" 2>&1 | tee "$LOG" \
