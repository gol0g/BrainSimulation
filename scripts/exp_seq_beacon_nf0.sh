...
LOG=/tmp/d22c_seq.log
cd ~/pygenn_test && rm -rf forager_brain_CODE CODE
echo "=== D22c beacon+nfood0 (음식충돌 제거) 25ep ==="
python -u "$RUN" --task integrated --seq-task --seq-nav --seq-goal-beacon \
  --inhib-wm -200 --zone-cx 0.3 --zone-cy 0.3 --sparse-reward --n-food 0 \
  --episodes 25 --seed 0 \
  --output "$D/d22c_beacon_nofood.json" 2>&1 | tee "$LOG" \
