#!/bin/bash
# C4 커리큘럼 사회 훈련: food_hidden 점진 램프(초반 가시→후반 은닉) 80ep + 저장.
...
W=$REPO/checkpoints/brain_social_curriculum_80ep.npz
LOG=/tmp/c4_curriculum.log
mkdir -p $REPO/checkpoints
cd ~/pygenn_test && rm -rf forager_brain_CODE CODE
echo "=== C4 커리큘럼 사회 훈련 80ep → $W ==="
python -u $REPO/backend/genesis/forager_brain.py --episodes 80 --render none \
