...
echo "=== D23 뇌 내부 목표항법 25ep ==="
python -u "$RUN" --task integrated --seq-task --seq-nav --seq-wm --seq-pattern-latch --seq-brain-nav \
  --inhib-wm -200 --zone-cx 0.3 --zone-cy 0.3 --sparse-reward --n-food 4 --episodes 25 --seed 0 \
  --output "$D/d23_brainnav.json" 2>&1 | tee /tmp/d23_seq.log \
