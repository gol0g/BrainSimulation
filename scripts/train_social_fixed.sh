...
W=$R/checkpoints/brain_social_fixed_80ep.npz
cd ~/pygenn_test && rm -rf forager_brain_CODE CODE
echo "=== C10 포화해소(-30)+사회과제+관찰회로 80ep ==="
python -u $R/backend/genesis/forager_brain.py --episodes 80 --render none \
  --social-task --mirror-motor 8.0 --sts-inhib -30 --save-weights "$W" 2>&1 | grep -iE "C5|Survival|saved|Traceback|Error" | tail -4
echo "=== 사회 프로브 ==="
